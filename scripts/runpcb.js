...
module.exports = async function runpcb({ dateStr, flags } = {}) {
  ...
  try { delete require.cache[require.resolve(pcbPath)]; } catch (_) {}

  const pcb = require(pcbPath);
  if (typeof pcb !== "function") {
    throw new Error(`runpcb: expected Engine Room/lib/pcb.js to export a function, got ${typeof pcb}`);
  }

  const md = await pcb({ dateStr, flags });
