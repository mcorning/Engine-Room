...
// tune to your vault (adjust if you use folders instead of tags)
...
function buildMonthDatesFromDays(baseMoment, days) {
  const out = [];
  for (const d of days) {
    // clone baseMoment; set day-of-month
    const m = window.moment(baseMoment).date(d);
