...
function normalizeDueDays(fm) {
  // Prefer "due_days"
  if (Array.isArray(fm.due_days))
    return fm.due_days
      .filter((n) => Number.isFinite(Number(n)))
      .map((n) => Number(n));
  if (typeof fm.due_days === 'number' && Number.isFinite(fm.due_days))
    return [fm.due_days];

  // Tolerate legacy "due_day"
  if (typeof fm.due_day === 'number' && Number.isFinite(fm.due_day))
    return [fm.due_day];

  // Tolerate legacy "dueDay"
  if (typeof fm.dueDay === 'number' && Number.isFinite(fm.dueDay))
    return [fm.dueDay];

  return [];
}
function normalizeDueDaysAny(fm) {
  // Accept due_days, due_day, dueDay (scalar or list)
  const v =
    fm.due_days ??
    fm.due_day ??
    fm.dueDay ??
    fm.pay_days ??
    fm.pay_day ??
    fm.payDay; // optional tolerance if you used different names
...
function normalizeDueAmounts(fm) {
  // Accept due_amounts or due_amount (scalar or list)
  const v = fm.due_amounts ?? fm.due_amount ?? fm.dueAmount;
  if (Array.isArray(v))
    return v.map(Number).filter((n) => Number.isFinite(n) || v.includes(0));
  const n = Number(v);
  return Number.isFinite(n) ? [n] : [];
}
...
function amountForIndex(fm, idx) {
  const arr = normalizeDueAmounts(fm);
  if (idx < arr.length && Number.isFinite(arr[idx])) return arr[idx];

  const fallback = baseAmount(fm);
  return fallback;
}
...
module.exports = async function pcb_rows(opts = {}) {
  if (typeof app === 'undefined') {
    throw new Error(
      'pcb_rows: global `app` is missing (not running inside Obsidian).'
    );
  }

  const dateStr = opts.dateStr;
  if (!dateStr)
    throw new Error('pcb_rows: opts.dateStr is required (YYYY-MM-DD).');
  ...
  for (const file of billFiles) {
    ...
    const bill = String(
      fm.ref ?? fm.bill ?? fm.name ?? path.basename(file, '.md')
    );
    ...
    const rawAmt =
      typeof fm.amount === 'number' && Number.isFinite(fm.amount)
        ? fm.amount
        : 0;
  ...
  for (const file of debtFiles) {
    const fp = path.join(debtsDir, file);
    const md = fs.readFileSync(fp, 'utf-8');
    const fm = parseFrontmatter(md);
    ...
    const dueDays = normalizeDueDaysAny(fm);
    const fallback = baseAmount(fm);

    // If there are no due days, skip (or choose to treat as “not scheduled”)
    if (!dueDays.length) continue;

    for (let i = 0; i < dueDays.length; i++) {
      const dueDay = dueDays[i];
      if (dueDay < 1 || dueDay > 31) continue;

      const due = new Date(Y, M - 1, dueDay);
      if (due.getMonth() + 1 !== M) continue; // skip invalid date like Feb 30

      const amt = amountForIndex(fm, i);

      // If amt is still 0 and you want to skip, do:
      // if (!amt) continue;

      // Debts are money OUT: negative
      const delta = amt > 0 ? -amt : amt;

      rows.push({
        bill, // or whatever you use for Flow label
        label:bill,
        due_date: ymd(due),
        on: ymd(due),
        amount: delta,
        autopay: fm.autopay === true,
        cycle: String(fm.cycle ?? 'monthly'),
        account: typeof fm.account === 'string' ? fm.account.trim() : '',
